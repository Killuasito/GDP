...
const EditWellModal = ({ isOpen, onClose, onUpdate, well }) => {
  ...
  useEffect(() => {
    if (isOpen && well) {
      setName(well.name || '');
      setStatus(well.status || 'active');
      setIsPasswordProtected(well.isPasswordProtected || false);
      setPassword('');
      setCurrentPassword('');
      checkEditPermission();

      // If well is password protected, fetch the existing password for admin/creator
      if (well.isPasswordProtected) {
  ...
  const checkEditPermission = async () => {
    ...
    try {
      const hasPermission = await firebaseService.hasEditPermission('wells', well.id);
  ...
  const fetchExistingPassword = async () => {
    ...
    try {
      ...
      const decryptedPassword = await firebaseService.getDecryptedPassword('wells', well.id);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isPasswordProtected !== well.isPasswordProtected ||
      ...
      toast.error(error.message || "Erro ao atualizar poço");
    } finally {
      setLoading(false);
    }
  };
                ...
                <div className={getPasswordSectionClasses()}>
                  ...
                  <div className={isPasswordProtected ? "transition-opacity duration-200" : "opacity-50 pointer-events-none transition-opacity duration-200"}>
                    ...
                    <p className="mt-2 text-sm text-gray-500">
                      Esta senha será necessária para acessar {well ? `o poço "${well.name}"` : 'o poço'}
                    </p>
                  </div>
                </div>
              )}
